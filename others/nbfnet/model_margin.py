...
class NBFNet_og(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dims, num_relation, message_func="distmult", aggregate_func="pna",
        ...
        for i in range(num_mlp_layer - 1):
            ...
            mlp.append(nn.ReLU())
        mlp.append(nn.Linear(feature_dim, input_dim, device=device))
    ...
    def forward(self, batch, train=True):
        ...
        feature = output
        # breakpoint()
        # index = batch.target_edge_index[1].unsqueeze(-1).expand(-1, -1, feature.shape[-1])
        ...
        # return feature_bh
        score = torch.softmax(self.mlp(feature_bh).squeeze(-1), axis=-1)
        # if train:
        #     neg_relations = self.get_negative_relations(self.layers[0].relation.weight, batch.target_edge_type, num_negative_samples=32)
        #     neg_scores = self.mlp(neg_relations).squeeze(-1)
        #     score = torch.cat([score.unsqueeze(-1), neg_scores], dim=-1)
        return score
        ...
        for epoch in range(epochs):
            ...
            for batch in train_loader:
                ...
                margin_loss = get_margin_loss_term(pred_br, relations_rh, batch.target_edge_type, 
                                                   num_negative_samples=10, margin=0.1, score_fn='xent', outs_as_left_arg=False)
                loss = margin_loss
            ...
            with torch.no_grad():
                ...
                for batch in val_loader:
                    ...
                    target_edge_type = batch.target_edge_type.reshape(-1, pred.shape[-1])
                    val_acc = get_acc_multihot(pred, target_edge_type, threshold=0.5)
        ...
        for data_filename in ['test_d', 'test_w', 'test_bl', 'test_opec']:
            ...
            with torch.no_grad():
                ...
                for batch in dataset_test_loader:
                    ...
                    rs_rh = torch.softmax(model.query.weight, axis=-1)
                    pred = -1*out_score(pred_bh, rs_rh, score_fn='xent', outs_as_left_arg=False)
                    target_edge_type = batch.target_edge_type.reshape(pred.shape[0], -1)
                    acc= get_acc_multihot(pred, target_edge_type, threshold=0.25)

                    preds = (pred.sigmoid() >= 0.25).to(pred.dtype)
                    f1_macro = f1_metric_macro(preds, target_edge_type)
                    f1_micro = f1_metric_micro(preds, target_edge_type)
                    num_data_points += target_edge_type.shape[0]
